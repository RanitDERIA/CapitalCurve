    ...
    try {
      ...
      try {
        ...
        try {
          ...
          setMessage("✅ Login successful! Click here to continue: http://localhost:3001/");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:3002/auth/login", // backend login route
        {
      ...
      if (err.code === 'ECONNABORTED') {
        ...
        setMessage("⚠️ Cannot connect to server. Please check if the backend is running on port 3002.");
      } else {
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="text-center mt-3">
          <small className="text-muted">
            Debug: Backend should be at port 3002, Main app at port 3001
          </small>
        </div>
      )}
    </div>
